...
class DatabaseHelper {
  ...
  Future<Database> _initDB() async {
    ...
    return await openDatabase(
      path,
      version: 2, // 1. Incrementa el número de versión
      onCreate: _onCreate,
      onUpgrade: _onUpgrade, // 2. Añade el callback de actualización
    );
  }
  ...
  // 3. Implementa el método onUpgrade
  // Esta función se llama automáticamente cuando la versión de la base de datos aumenta.
  Future _onUpgrade(Database db, int oldVersion, int newVersion) async {
    // Comprueba si la versión antigua es menor que 2 (la nueva versión).
    if (oldVersion < 2) {
      // Si es así, añade las columnas que faltan a la tabla existente.
  ...
  Future<List<Sensor>> getUniqueSensors() async {
    ...
    return List.generate(maps.length, (i) {
      return Sensor(
        ...
        color: Colors.blueGrey, // El color no se guarda, se asigna dinámicamente
      );
    });
  }
